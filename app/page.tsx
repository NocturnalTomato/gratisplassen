...
import PreciseLocationPrompt, { markLocationAccuracy } from "@/components/PreciseLocationPrompt";
import type { LocationWithStats } from "@/lib/types";
import type { MapBounds, SearchFocus } from "@/components/MapView";

const MapView = dynamic(() => import("@/components/MapView"), {
  ssr: false,
  loading: () => <div className="flex h-full items-center justify-center bg-rose-50 text-gray-400 dark:bg-zinc-950 dark:text-zinc-500">Kaart laden…</div>,
});
...
export default function Home() {
  ...
  async function loadLocations(
    ...
    try {
      ...
      if (opts?.isSearch && lat !== undefined && lon !== undefined) {
        ...
        // Een gebiedszoekopdracht (plaats/gemeente/provincie/land) heeft een
        // bbox en zoomt uit op dat hele gebied — de "geen toilet vlakbij"
        // waarschuwing gaat dan over het exacte zoekpunt, niet over het
        // gebied, en is dus verwarrend. Alleen tonen bij een puntzoekopdracht
        // (adres/postcode/"gebruik mijn locatie").
        if (opts.bbox) {
  ...
  function useMyLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        markLocationAccuracy(pos.coords.accuracy);
        setUserPos(p);
  ...
  async function submitManual(e: React.FormEvent) {
    ...
    try {
      ...
      setUserPos(p);
      boundsRef.current = null;
      await loadLocations(p.lat, p.lon, { isSearch: true, bbox: geo.bbox });
    } catch {
  ...
  const list = (
    <>
      {status === "loading" && <p className="text-sm text-gray-400 dark:text-zinc-500">Laden…</p>}
  ...
  return (
    <main className="relative h-[100dvh] w-full overflow-hidden bg-rose-50 dark:bg-zinc-950">
      ...
      <WelcomeModal />
      <PreciseLocationPrompt onRetry={useMyLocation} />
      ...
      {!showAddForm && (
        ...
        >
          <div className="pointer-events-auto flex w-full max-w-2xl flex-col gap-2 rounded-2xl bg-white/95 p-3 shadow-lg backdrop-blur dark:bg-zinc-900/95">
            <div className="flex items-center justify-between gap-2">
              <div className="flex min-w-0 items-center gap-1.5">
                <h1 className="truncate text-base font-black text-rose-600 dark:text-rose-400 sm:text-lg">🚽 Gratis Plassen</h1>
                <CoffeeMenu />
              </div>
              <div className="flex shrink-0 items-center gap-2">
                <button
                  onClick={openAddForm}
                  className="rounded-full border border-rose-300 px-3 py-1.5 text-xs font-semibold text-rose-600 hover:bg-rose-50 dark:border-rose-800 dark:text-rose-300 dark:hover:bg-zinc-800 sm:text-sm"
                >
                  ➕ Toilet toevoegen
                </button>
                <HamburgerMenu filterUrinals={filterUrinals} onFilterUrinalsChange={setFilterUrinals} />
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-2">
                ...
                <form onSubmit={submitManual} className="relative flex flex-1 min-w-[180px] gap-2">
                  <input
                    ...
                    autoComplete="off"
                    className="w-full min-w-0 rounded-full border border-gray-300 bg-white px-3 py-1.5 text-xs dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100 dark:placeholder:text-zinc-500 sm:text-sm"
                  />
                  <button
                    type="submit"
                    className="shrink-0 rounded-full border border-rose-300 px-3 py-1.5 text-xs text-rose-600 hover:bg-rose-50 dark:border-rose-800 dark:text-rose-300 dark:hover:bg-zinc-800 sm:text-sm"
                  >
                    Zoek
                  </button>
                  {showSuggestions && suggestions.length > 0 && (
                    <ul className="absolute left-0 right-14 top-full z-20 mt-1 max-h-56 overflow-y-auto rounded-xl border border-gray-200 bg-white py-1 shadow-lg dark:border-zinc-700 dark:bg-zinc-800">
                      {suggestions.map((s) => (
                        <li key={s.id}>
                          <button
                            type="button"
              ...
              {wildplasCheck && (
                <div
                  className={`rounded-lg px-3 py-2 text-xs ${
                    wildplasCheck.magWaarschijnlijk
      ...
      {!showAddForm && (
        <div
          className="absolute bottom-4 left-4 z-10 hidden w-[360px] flex-col overflow-hidden rounded-2xl bg-white shadow-lg dark:bg-zinc-900 lg:flex"
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-3">
            {list}
            <p className="mt-3 text-center text-[11px] leading-snug text-gray-400 dark:text-zinc-500">
